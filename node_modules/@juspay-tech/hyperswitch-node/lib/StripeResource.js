...
StripeResource.prototype = {
        ...
        .then(
          (jsonResponse) => {
            if (jsonResponse.error) {
              ...
              if (statusCode === 401) {
                err = new HyperAuthenticationError(jsonResponse.error);
              } else if (statusCode === 403) {
                err = new StripePermissionError(jsonResponse.error);
              } else if (statusCode === 429) {
                err = new StripeRateLimitError(jsonResponse.error);
              } else {
                err = HyperError.generate(jsonResponse.error);
              }
          ...
          (e) => {
            throw new HyperAPIError({
              message: 'Invalid JSON received from the Hyper API',
              exception: e,
  ...
  _generateConnectionErrorMessage(requestRetries) {
    return `An error occurred with our connection to Hyper.${
